...
>>>>>>> 9a39cb08adf4cffd64c743717d0c174f580bed0c
    def __init__(self):
...
>>>>>>> 9a39cb08adf4cffd64c743717d0c174f580bed0c
    ...
    def _do_align(self, observed_word, intended_word):
        ...
        table = zeros(
            (M+1, N+1), dtype=[('cost', float64), ('backtrace', int8)])

        for i in range(1, M+1):
            table[i, 0] = (table['cost'][i-1, 0] +
                           self.ins_cost(observed_word[i-1]), self.INS)
        for j in range(1, N+1):
            table[0, j] = (table['cost'][0, j-1] +
                           self.del_cost(intended_word[j-1]), self.DEL)

        for i in range(1, M+1):
            for j in range(1, N+1):
                this_ins = (table['cost'][i-1, j] +
                            self.ins_cost(observed_word[i-1]), self.INS)
                this_del = (table['cost'][i, j-1] +
                            self.del_cost(intended_word[j-1]), self.DEL)
                this_sub = (table['cost'][i-1, j-1] + self.sub_cost(
                    observed_word[i-1], intended_word[j-1]), self.SUB)
...
<<<<<<< HEAD
    ...
    def sub_cost(self, observed_char, intended_char):
        if observed_char == intended_char:
            ...
            return 1-self.probs[intended_char][observed_char]
...
>>>>>>> 9a39cb08adf4cffd64c743717d0c174f580bed0c
    ...
    def pretty_print(self, observed, correct):
        ...
        print("Distance between '{}' and '{}' is {:.5f}".format(
            observed, correct, dist))
...
<<<<<<< HEAD
    parser.add_argument(
        "--store", "-s", type=argparse.FileType('wb'), required=True)
    parser.add_argument(
